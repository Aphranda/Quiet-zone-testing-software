...
try:
    import serial
except ImportError:  # pragma: no cover - handled at runtime for clearer errors.
    serial = None  # type: ignore[assignment]

from lcd_link.routing import LinkRouter, switch_box_profile_from_commands
...
class Lcd74000fSwitchBoxController:
    ...
    def connect(self) -> InstrumentInfo:
        ...
        try:
            self._idn = self.query_command(self._config.identify_command)
        except Exception:
            self.disconnect()
            raise
    ...
    def query_command(self, command: str) -> str:
        self._ensure_connected()
        return self._query_checked(command)
    ...
    def _open_serial(self) -> None:
        if serial is None:
            raise Lcd74000fSwitchBoxError("缺少 pyserial 依赖，无法打开串口。")
