...
- [ ] P1-01 抽出 `FilenamePolicy`：文件名清洗、探头偏置 tag、trace 文件名规则从 `InstrumentService` 移出并可单元测试。
- [ ] P1-02 抽出 `TraceStorage`：CSV 保存、trace index、metadata 输出从 `InstrumentService` 移出；原扫描输出不回归。
