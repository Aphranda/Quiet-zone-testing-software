...
- [x] P5-01 统一仪器默认值来源：新增仪器默认配置模块，VNA、扫描架、开关箱的 IP、端口、超时、波特率、轴号、每毫米单位和默认速度只保留一个权威定义。
- [x] P5-02 收口 `ConnectionPanel` 默认值：控件构造不再直接写业务默认值，初始 UI 状态全部来自 `ConnectionViewModel` 或统一默认配置。
- [x] P5-03 清理 S 参数链路旧语义：S 参数只属于 VNA 测量；开关箱主链路接口改为极化、DUT 目标和原始命令，`select_s_parameter()` 仅保留 deprecated 兼容路径或移除。
- [x] P5-04 更新链路文档：README、链路设计文档和迁移方案不再把“按 S 参数切换开关箱”描述为当前行为。
- [x] P5-05 抽出实时曲线标签识别模型：主线条 X/Y、位置 L/R/U/D/M 和四分之一边界判断迁入 presentation model，并修正非 0 起点扫描区间的识别逻辑。
- [x] P5-06 继续瘦身 `MainWindow`：DUT 链路命令拼接、任务结果处理、状态栏和日志路由逐步迁入 application/presentation controller。
- [x] P5-07 整理当前项目样式：从参考 `style.css` 中提炼当前项目实际使用规则，避免长期依赖末尾覆盖大字号和无关控件规则。
- [x] P5-08 建立收口回归验证：每完成 P5 子任务后运行全量 `unittest`，涉及 UI 时补充主窗口 offscreen 初始化检查。

### P5 验收

- 仪器默认值修改只需要改一个模块，UI、domain、factory 和 hardware config 不再漂移。
- 开关箱链路不再与 S 参数绑定，文档和代码命名一致表达当前行为。
- 实时曲线标签识别可用单元测试覆盖，并支持非 0 起点扫描区间。
