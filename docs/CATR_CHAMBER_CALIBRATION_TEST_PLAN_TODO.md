...
- 删除不应存在的 `AUX_CORR` 概念。
- 辅助线原始实测量记为 `S21_AUX_*`，通常为负值；参与计算前转换为线缆损耗量 `L_AUX_* = -S21_AUX_*`，公式中使用 `+ L_AUX_*`。
...
- 记录并修复 HTML 评审中发现的链路/参数冲突：
  ...
  - `LINK-CAL-003` 增加整机发射 SA 接收侧修正参数 `L_SYS_TX_SA_H/V` 与 `L_SYS_TX_SA_H/V_AMP1`，避免 `LINK-SYS-001` 误用 `L_DUT_SA`。
  - `LINK-SYS-001` 链路箱指令只开通 `H/V, SA` 或 `H/V, AMP1, SA`，不再同时下发 `DUT, SA`。
...
- [ ] 确认 `LINK-CAL-003` 中 `L_DUT_SA` / `L_DUT_SA_AMP1` 仅用于 DUT 到 SA 类链路；整机发射 SA 接收侧已改用 `L_SYS_TX_SA_H/V` / `L_SYS_TX_SA_H/V_AMP1`。
...
- [ ] 公式必须按带符号 dB 逐频点计算。
- [ ] 辅助线原始实测 `S21_AUX_*` 先转换为线缆损耗量 `L_AUX_* = -S21_AUX_*`，再参与 `+ L_AUX_*` 计算。
...
- [ ] 测试项页面显示当前将使用的路损文件：
  ...
  - `LINK-SYS-001` 使用 `L_SYS_TX_SA_H/V` 或 `L_SYS_TX_SA_H/V_AMP1`。
...
- [ ] 为校准项定义增加单元测试：
  ...
  - 禁止指令不被生成。
- [ ] 为路损公式增加单元测试：
  - `S21_AUX_*` 到 `L_AUX_* = -S21_AUX_*` 的转换与 `+ L_AUX_*` 计算。
  - `G_STD_HORN` 正值减法。
  - AMP 链路结果可为正值。
  - 频段交集命名。
- [ ] 为链路箱命令生成增加测试：
