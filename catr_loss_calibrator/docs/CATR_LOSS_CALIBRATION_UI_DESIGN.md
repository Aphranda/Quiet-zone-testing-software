...
校准过程中的上一步、下一步、跳过、重试、取消不放在步骤执行区常驻显示，而是放入当前确认提示窗：

- “确认接线完成”提示窗：确认接线完成、下一步、取消校准。
- “确认数据保存完成”提示窗：完成并继续、上一步、下一步、取消校准。
- 上一步表示回到该小步骤的接线确认并重复测量。
- 下一步表示跳过当前小步骤并进入下一个小步骤。
...
5. UI 显示当前步骤和接线说明。
6. 操作员完成接线。
7. 在“确认接线完成”提示窗中点击“确认接线完成”。
8. runner 执行链路切换、采样、保存。
9. 在“确认数据保存完成”提示窗中选择“完成并继续”、“上一步”、“下一步”或“取消校准”。
10. UI 更新当前状态、结果摘要和 LOG。
11. 重复直到结束。

### 6.3 异常流程

- 接线错误：在保存确认提示窗点击“上一步”，回到该小步骤的接线确认并重测。
- 操作员临时离开：后续应支持“暂停”。
- 发现配置不对：允许“取消”。
- 当前小步骤可跳过时：在提示窗点击“下一步”。
...
- `WAIT_MANUAL_CONFIRM`：通过当前提示窗允许确认接线完成、完成并继续、上一步、下一步或取消校准。
